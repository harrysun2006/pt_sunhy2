$(function(){	
	...
	$('#btnReset').click(function(){
		window.location = BETTER_ADMIN_URL+'/venue';
	});
	$("input[name^='accessall_']").click(function(){
		
		var id = $(this).attr('name').replace('accessall_','');		
		$.post(BETTER_ADMIN_URL+'/venue/checkvenue', {
			'id': id,
			'doing': 1,		
			'dospec':1
		}, function(json){
	...
	$("input[name^='accessreq_']").click(function(){	
		var id = $(this).attr('name').replace('accessreq_','');	
		$.post(BETTER_ADMIN_URL+'/venue/checkvenue', {
			'id': id,
			'doing': 1,		
			'dospec' :0
