...
class Mobile_SearchController extends Better_Mobile_Front {
	...
	public function changeprovinceAction() {		
		...
		if($checked){			
			$return['has_err'] = 0;		
			$this->_redirect('/mobile/search/changecity');
		} 
		return $return;
	}
	public function changecityAction() {
		...
		$this->view->cityArray =Better_Citycenterll::$cityArray;
		if(!$search_province){
			$this->_redirect('/mobile/search/changeprovince');
		}		
		foreach($this->view->cityArray as $rows){	
