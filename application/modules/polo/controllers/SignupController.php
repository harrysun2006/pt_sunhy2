...
class Polo_SignupController extends Better_Mobile_Front {
	...
	public function doAction()
	{
		...
		if (count($err)>0) {
			...
			if (!$uid) {
				...
				$this->_helper->getHelper ( 'Redirector' )->gotoUrl ( '/polo/signup/step2' );
	...
	{		
		...
		if (count($post)==0) {
			$this->_helper->getHelper('Redirector')->gotoUrl('/polo/signup/step3');
			$post = $this->user->getUserInfo();
		} else {
			$btn = $post['btn'];
			$hasErr = false;
