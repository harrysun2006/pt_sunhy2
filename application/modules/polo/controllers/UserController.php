...
class Polo_UserController extends Better_Mobile_Front {
	...
	public function indexAction() {
		...
		if( $uid != $this->uid ){
			...
			if( $this->isFollower( $uid ) ){
				$this->view->isFollowerLabel = $this->view->lang->user->follow->cancel; //"取消关注";
				$this->view->isAddFollow = False;
			}else{
				$this->view->isFollowerLabel = $this->view->lang->mobile->user->following->noun; 				
				$this->view->isAddFollow = True;
			}		
    	...
    	if($user_state=='banned'){
			$this->_redirect('/polo/user/close');	
    	}	
		$checkthis = ($this->view->userInfo['priv_blog']=="1" && !$this->view->isTa && !$this->isFollower($uid));		
		if($checkthis){
			$thisuser = Better_User::getInstance($uid)->getUserInfo();
			$this->view->error_info=$this->lang->javascript->user->must_follow_part1.$thisuser['nickname'].$this->lang->javascript->user->must_follow_part2;
		} else {			
			...
			for($i=0;$i<count($timeline["rows"]);$i++){		
				...
				} else if($timeline["rows"][$i]["type"]!='checkin' && strlen($timeline["rows"][$i]["message"])==0 && strlen($timeline["rows"][$i]["upbid"])!='0'){
	...
	public function postAction(){
		...
		if($checkthis){
			...
			if( $page * self::$maxActivities < $total )
				$this->view->urlNext = "<a href=\"/polo/user/post?uid=".$uid."&page=".($page+1)."\">".$this->lang->mobile->pager->next."</a>";
			if( $page > 1 )
				$this->view->urlPrev = "<a href=\"/polo/user/post?uid=".$uid."&page=".($page-1)."\">".$this->lang->mobile->pager->pre."</a>";
		}
	}
	...
	public function shoutAction(){
		...
		if( $poi_id ){
			...
			$placeInfo = "@<a href=\"/polo/place?pid=".$poi_id."\">".$place['name']."</a>";			
		}
		if($upbid){
			$category = "normal";
		}
	...
	public function doshoutAction(){
		...
		if( $success ){
			...
			$this->_redirect('/polo/user?uid='.$this->uid.'&err='.$err);			
			...
			$redirecturl = '/polo/user/shout?cat='.$post['type'].'&error='.$this->lang->mobile->global->postnot_null;
			if($post['upbid']){
	...
	public function docheckinAction(){
		...
		if( !$this->view->lastcheckin['poi']['poi_id'] )
			$this->_redirect('/polo/search?cat=place');
	}
	...
	public function crownAction(){
		...
		$this->view->isTa = ($uid == $this->uid)?True:False;	
		$this->view->userInfo = Better_User::getInstance($uid)->getUserInfo();	
		$checkthis = ($this->view->userInfo['priv_blog']=="1" && !$this->view->isTa && !$this->isFriend($uid));		
		if($checkthis){		
			$thisuser = Better_User::getInstance($uid)->getUserInfo();
			...
			$crowns = Better_User_Major::getInstance($uid)->getAll(1, 20);
	...
	public function badgeAction(){
		...
		if($checkthis){			
			...
			if( $page * $pagenum < $total )
				$this->view->urlNext = "<a href=\"/polo/user/badge?uid=".$uid."&page=".($page+1)."\">".$this->lang->mobile->pager->next."</a>";
			if( $page > 1 )
				$this->view->urlPrev = "<a href=\"/polo/user/badge?uid=".$uid."&page=".($page-1)."\">".$this->lang->mobile->pager->pre."</a>";
	...
	public function checkinAction(){
		...
		if($checkthis){
			...
			$this->view->start = ( $page - 1 ) * self::$maxActivities + 1;
			if( $page < $totalPages )
				$this->view->urlNext = "<a href=\"/polo/user/checkin?uid=".$uid."&page=".($page+1)."\">".$this->lang->mobile->pager->next."</a>";
			if( $page > 1 )
				$this->view->urlPrev = "<a href=\"/polo/user/checkin?uid=".$uid."&page=".($page-1)."\">".$this->lang->mobile->pager->pre."</a>";
		}
	}
	...
	public function followerAction(){
		...
		if($uid==BETTER_SYS_UID){
			...
			if($checkthis){			
				...
				$this->view->start = ( $page - 1 ) * self::$maxActivities + 1;
				if( $page < $totalPages )
					$this->view->urlNext = "<a href=\"/polo/user/follower?uid=".$uid."&page=".($page+1)."\">".$this->lang->mobile->pager->next."</a>";
				if( $page > 1 )
					$this->view->urlPrev = "<a href=\"/polo/user/follower?uid=".$uid."&page=".($page-1)."\">".$this->lang->mobile->pager->pre."</a>";
			}
		}
	}
	...
	public function followingAction(){
		...
		if($checkthis){			
			...
			$this->view->start = ( $page - 1 ) * self::$maxActivities + 1;
			if( $page < $totalPages )
				$this->view->urlNext = "<a href=\"/polo/user/following?uid=".$uid."&page=".($page+1)."\">".$this->lang->mobile->pager->next."</a>";
			if( $page > 1 )
				$this->view->urlPrev = "<a href=\"/polo/user/following?uid=".$uid."&page=".($page-1)."\">".$this->lang->mobile->pager->pre."</a>";
		}
	}
	...
	public function friendAction(){
		...
		if($checkthis){			
			...
			$this->view->start = ( $page - 1 ) * self::$maxActivities + 1;
			if( $page < $totalPages )
				$this->view->urlNext = "<a href=\"/polo/user/friend?uid=".$uid."&page=".($page+1)."\">".$this->lang->mobile->pager->next."</a>";
			if( $page > 1 )
				$this->view->urlPrev = "<a href=\"/polo/user/friend?uid=".$uid."&page=".($page-1)."\">".$this->lang->mobile->pager->prev."</a>";
		}
	}
	...
	public function favoriteAction(){
		...
		$this->view->start = ( $page - 1 ) * self::$maxActivities + 1;	
		$poiurl   = 	"<a href=\"/polo/user/favorite?cat=poi\">".$this->lang->mobile->poi_favorites."</a>";
		$tipsurl  =  "<a href=\"/polo/user/favorite?cat=tips\">".$this->lang->mobile->tips_favorites."</a>";
		$shouturl = "<a href=\"/polo/user/favorite?cat=normal\">".$this->lang->mobile->blog_favorites."</a>";
		switch($cat)	{
			case "poi":
		...
		if( $page * self::$maxActivities < $total ){
				$this->view->urlNext = "<a href=\"/polo/user/favorite?cat=".$cat."&page=".($page+1)."\">".$this->lang->mobile->pager->next."</a>";
		}
		if( $page > 1 ){
				$this->view->urlPrev = "<a href=\"/polo/user/favorite?cat=".$cat."&page=".($page-1)."\">".$this->lang->mobile->pager->pre."</a>";
		}	
	...
	public function addfriendAction(){
		...
		if((float)$this->user->karma<0){
			$error = -20;	
			$this->_redirect('/polo/user?uid='.$fid.'&err='.$error);			
			exit;
		} 
		if($fid && $fid!=BETTER_SYS_UID){
			...
			$this->_redirect('/polo/user?uid='.$this->uid.'&err='.$error);
		}
	}
	...
	public function addblockAction(){
		...
		if( $uid ){
			...
			$this->_redirect('/polo/user?uid='.$uid);
		}else{
			$this->_redirect('/polo/user?uid='.$this->uid);
		}
	}
	...
	public function tipsAction(){
		...
		$this->view->start = ( $page - 1 ) * self::$maxActivities + 1;
		if( $page * self::$maxActivities < $total )
			$this->view->urlNext = "<a href=\"/polo/user/tips?uid=".$uid."&page=".($page+1)."\">".$this->lang->mobile->pager->next."</a>";
		if( $page > 1 )
			$this->view->urlPrev = "<a href=\"/polo/user/tips?uid=".$uid."&page=".($page-1)."\">".$this->lang->mobile->pager->pre."</a>";
	}
	...
	public function pollAction(){
		...
		if($poiId>0){
			$this->_redirect('/polo/place?pid='.$poiId."&err=".$result);
		} else {
			$this->_redirect('/polo/user/tips?uid='.$blog['uid']."&err=".$result);
		}
	}
	...
	public function doexchangebadgeAction(){
		...
		$this->_redirect('/polo/user/showbadge?badge_id='.$id.'&err='.$this->view->errorinfo);			
