...
class Polo_PlaceController extends Better_Mobile_Front {
	...
	public function indexAction() {
		...
		if($place['closed']==1){
			...
			$this->_redirect('/polo/place/close');
		} else {			
			...
			for($i=0;$i<count($tips["rows"]);$i++){
				...
				} else if($tips["rows"][$i]["type"]!='checkin' && strlen($tips["rows"][$i]["message"])==0 && strlen($tips["rows"][$i]["upbid"])!='0'){
	...
	public function tipsAction(){
		...
		if( $page < $totalPages )
			$this->view->urlNext = "<a href=\"/polo/place/tips?pid=".$poi_id."&page=".($page+1)."\">".$this->lang->mobile->pager->next."</a>";
		if( $page > 1 )
			$this->view->urlPrev = "<a href=/polo/place/tips?pid=".$poi_id."&page=".($page-1)."\">".$this->lang->mobile->pager->pre."</a>";
	}
	...
	public function nearbyusersAction(){
		...
		if( !$lat && !$lon ){
			...
			echo $this->view->render('/polo/place/nearbyusers');
			exit(0);
		}
		...
		if( $page * self::$maxNumActivities < $total )
			$this->view->urlNext = "<a href=\"/polo/place/nearbyusers?pid=".$poi_id."&page=".($page+1)."\">".$this->lang->mobile->pager->next."</a>";
		if( $page > 1 )
			$this->view->urlPrev = "<a href=/polo/place/nearbyusers?pid=".$poi_id."&page=".($page-1)."\">".$this->lang->mobile->pager->pre."</a>";
	}
