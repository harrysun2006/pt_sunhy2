 ...
 *
 */
class Public_StatusesController extends Better_Controller_Public
{
	...
	public function friendstimelineAction()
	{
		...
		$rows = $this->user->status()->apiPublicTimeline(array(
			'page' => $this->page,
	...
	public function searchAction()
	{
		...
		$query = urldecode(trim($this->getRequest()->getParam('query', '')));
	...
	public function placetimelineAction()
	{
		...
		$lat = $this->getRequest()->getParam('lat', 0);
		$lon = $this->getRequest()->getParam('lon', 0);
...
//			))->search();
	...
	public function updateAction()
	{
		...
		$status = trim(urldecode($this->post['content']));
		...
		$source = urldecode($this->post['source']);
		...
		if ($this->todo=='create') {
			...
			if (Better_Blog::validBid($bid)) {
				...
				if (isset($data['blog']['bid'])) {
					if ($data['blog']['uid']) {
						$f = Better_User_Favorites::getInstance($this->uid)->add($data['blog']['bid'], $data['user']['uid']);
						if ($f===true) {
							$this->user->favorites[] = $data['blog']['bid'];
	...
	{		
		...
		if (is_array($_FILES) && isset($_FILES['photo'])) {
			...
			if (is_array($result) && $result['file_id']) {
				...
				$photo = $result;
				$theend =  Better_Attachment_Parse::getInstance($result)->result();
				$this->data[$this->xmlRoot] = &$theend;				
			} else {
				$this->errorDetail = __METHOD__.':'.__LINE__;
				Better_Controller::sendSquidHeaderC($result);
				$this->error('error.status.upload.code_'.$result);
