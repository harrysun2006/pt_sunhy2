...
/**
 * 用户缓存清理队列处理
 * 由于开开有逐步将用户吼吼、签到、贴士逐条放在memcached中的趋势，而用户的某些操作需要更新某些缓存的cache值
 * 
 * @package scripts
...
if (count($args)>0 && $args[1]) {
	$actTypes = $args[1];
	define('PID_FILE', dirname(___FILE__).'/clear_cache_'.$actTypes.'.pid');
} else {
	$actTypes = array(
		0
		);
	define('PID_FILE', dirname(__FILE__).'/clear_cache.pid');
}
...
while(true) {
	...
	try {
		$module = Better_Queue_Module_Clearcache::getInstance();
		...
		if ($data['queue_id']) {
			...
			if ($result) {
				$module->complete($data['queue_id']);
			} else {
				$module->complete($data['queue_id'], -1);
				Better_Log::getInstance()->logInfo(serialize($data), 'clearcache_error', true);
			}
			
			echo $data['queue_id']." done.\n";	
		} 
	} catch (Exception $e) {
		echo $e->getMessage()."\n".$e->getTraceAsString()."\n";
		Better_Log::getInstance()->logInfo("\n========================================\nAct Types : ".serialize($actTypes)."\nError Message : ".$e->getMessage()."\nTrace: ".$e->getTraceAsString(), 'clearcache_exception', true);
		exit(0);
	}
