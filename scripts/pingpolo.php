...
define('THIS_PID', dirname(__FILE__) . '/pingpolo.pid');
...
while(true) {
	try {
		$rows = Better_DAO_PingQueuepolo::getInstance()->popupQueue(20);
		...
		if (is_array($rows) && count($rows)>0) {
			foreach ($rows as $row) {
				...
				Better_DAO_PingQueuepolo::getInstance()->updateByCond(array(
					'send_time' => time()
				), array(
