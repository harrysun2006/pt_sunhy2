...
foreach ($rows as $row) {
	...
	$uid = $row['uid'];	
	if (!in_array($protocol, $protocols) || $oauth_token || $password == 'abc123') continue;
	...
	if ($service->getToken()) {
		...
		$_tid && Better_DAO_ThirdBinding::getInstance($uid)->update(array('tid' => $_tid, 'oauth_token' => $_accecss_token, 'oauth_token_secret' => $_accecss_token_secret), $where);	
		echo "$i,$uid,$protocol,$_tid,$_accecss_token,$_accecss_token_secret\r\n";
		$i++;			
	}
}
