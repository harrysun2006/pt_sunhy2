...
foreach($sids as $sid) {
	...
	$start = gmmktime(16, 0, 0, 11, 30, 2010);
	
        $sql = "SELECT a.regtime,a.email,b.username,a.uid,b.dateline FROM better_account as a,better_3rdbinding as b  where a.regtime>='".$start."' and a.uid=b.uid and b.protocol='sina.com'";
        $rs = Better_DAO_Base::squery($sql, $rdb);
        $rows = $rs->fetchAll();
       
        foreach ($rows as $row) {
        	$timescheck = $row['dateline']-$row['regtime'];
        	if($row['email']==$row['username'] && $timescheck<300){
        		$k++;
        		echo $k."\n";
        	}
        	$tempname = 'kai'.$row['uid'].'@k.ai';
       		if($row['username']==$tempname &&  $timescheck<300){
        		$i++;
        		echo $i."\n";
        	}
		}
...
echo "Sina2kai:".$k." Kai2Sina:".$i."Done.\n";
exit(0);
