...
//	进程锁
define('SYNC_BLOG_LOCK', dirname(__FILE__).'/fpr.lock');
...
foreach($sids as $sid) {
	...
	$select = $rdb->select();
	$select->from(BETTER_DB_TBL_PREFIX.'blog');
	...
	foreach ($rows as $row) {
		...
		$select = $rdb->select();
		$select->from(BETTER_DB_TBL_PREFIX.'poi_poll');
		$select->where('blog_id=?', $bid);
		...
		$sql = "UPDATE `".BETTER_DB_TBL_PREFIX."blog` SET poll_result='".intval($ups-$downs)."', up='".intval($ups)."', down='".intval($downs)."' WHERE `bid`='".$bid."'";
		echo $sql."\n";
