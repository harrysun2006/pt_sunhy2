...
define('THIS_PID', dirname(__FILE__).'/emailer2.pid');
...
while(true) {
	$rows = Better_DAO_EmailCommonQueue::getInstance()->getAllQueue();
	...
	foreach ($rows as $row) {
		try {
			...
			try{
				if($flag && $row['type']=='nologin'){
					Better_DAO_Emailuser::getInstance($row['uid'])->reconnection()->update(array('lastloginemail'=>time()), $row['uid']);
				}else if($flag && $row['type']=='novalidate'){
					Better_DAO_Emailuser::getInstance($row['uid'])->reconnection()->update(array('lastvalidteemail'=>time()), $row['uid']);
				}
			
			}catch(Exception $ee){
				Better_Log::getInstance()->logAlert('SEND_EMAIL_EXCEPTION_AT_LINE('.__LINE__.'):['.$ee->getMessage()." =========== \n".$ee->getTraceAsString().']', 'email2');
			}
			
			Better_DAO_EmailCommonQueue::getInstance()->reconnection()->updateByCond(array(
				'tried' => $row['tried']+1,
				'send_time' => time(),
				'result' => $result,
				), array(
			...
			if ($result=='FAILED') {
	 		...
	 		try {
		 		Better_DAO_EmailCommonQueue::getInstance($row['uid'])->reconnection()->updateByCond(array(
		 			'tried' => $row['tried']+1,
		 			'result' => $result,
		 			'send_time'=>time()
		 			), array(
	 		...
	 		Better_Log::getInstance()->logAlert('SEND_EMAIL_EXCEPTION_AT_LINE('.__LINE__.'):['.$ex->getMessage()." =========== \n".$ex->getTraceAsString().']', 'email');
	 		//exit(0);
	 	}		
	}
	if (count($rows) < 200) {//默认取200条消息，如果小于200条，则终止循环
		break;
	}
	touch(THIS_PID);
	sleep(10);
