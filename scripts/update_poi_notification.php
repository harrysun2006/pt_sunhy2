...
define('BETTER_NEED_INVITECODE', true);
	...
	$args = $_SERVER['argv'];
	$filename = $args[1] ? $args[1] : 'closed_notifi.csv';
		...
		try{
			...
			foreach ($rows as $row){
				if($row[0] && $row[1] && $row[2]){
					if($row[0]=='update'){
						list($old, $new) = explode('-', $row[2]);
						if($old && $new){
							Better_DAO_Poi_Notification::getInstance()->updateByCond(array('poi_id'=>$new), array('nid'=>$row[1], 'poi_id'=>$old));
						}
					}else if($row[0]=='delete'){
						Better_DAO_Poi_Notification::getInstance()->delete($row[1]);
					}else{
						echo 'wrong type';
					}
				}
