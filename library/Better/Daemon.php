...
/**
 * Better_Daemon. Create daemons with practicle functions 
 * like Better_Daemon::start()
 *
...
class Better_Daemon
{
    ...
    /**
     * System is unusable (will throw a Better_Daemon_Exception as well)
     */
    const LOG_EMERG = 0;
    
    /**
     * Immediate action required (will throw a Better_Daemon_Exception as well)
     */ 
    const LOG_ALERT = 1;
    
    /**
     * Critical conditions (will throw a Better_Daemon_Exception as well)
     */
    const LOG_CRIT = 2;
    ...
    static protected $_optionDefinitions = array(
        ...
        'usePEARLogInstance' => array(
            ...
            'detail' => 'This will replace Better_Daemon\'s own logging facility',
            'required' => true,
        ),
    ...
    static protected $_sigHandlers = array(
        SIGHUP => array('Better_Daemon', 'defaultSigHandler'),
        SIGINT => array('Better_Daemon', 'defaultSigHandler'),
        SIGQUIT => array('Better_Daemon', 'defaultSigHandler'),
        SIGILL => array('Better_Daemon', 'defaultSigHandler'),
        SIGTRAP => array('Better_Daemon', 'defaultSigHandler'),
        SIGABRT => array('Better_Daemon', 'defaultSigHandler'),
        'SIGIOT' => array('Better_Daemon', 'defaultSigHandler'),
        SIGBUS => array('Better_Daemon', 'defaultSigHandler'),
        SIGFPE => array('Better_Daemon', 'defaultSigHandler'),
        SIGUSR1 => array('Better_Daemon', 'defaultSigHandler'),
        SIGSEGV => array('Better_Daemon', 'defaultSigHandler'),
        SIGUSR2 => array('Better_Daemon', 'defaultSigHandler'),
        SIGPIPE => SIG_IGN,
        SIGALRM => array('Better_Daemon', 'defaultSigHandler'),
        SIGTERM => array('Better_Daemon', 'defaultSigHandler'),
        'SIGSTKFLT' => array('Better_Daemon', 'defaultSigHandler'),
        'SIGCLD' => array('Better_Daemon', 'defaultSigHandler'),
        'SIGCHLD' => array('Better_Daemon', 'defaultSigHandler'),
        SIGCONT => array('Better_Daemon', 'defaultSigHandler'),
        SIGTSTP => array('Better_Daemon', 'defaultSigHandler'),
        SIGTTIN => array('Better_Daemon', 'defaultSigHandler'),
        SIGTTOU => array('Better_Daemon', 'defaultSigHandler'),
        SIGURG => array('Better_Daemon', 'defaultSigHandler'),
        SIGXCPU => array('Better_Daemon', 'defaultSigHandler'),
        SIGXFSZ => array('Better_Daemon', 'defaultSigHandler'),
        SIGVTALRM => array('Better_Daemon', 'defaultSigHandler'),
        SIGPROF => array('Better_Daemon', 'defaultSigHandler'),
        SIGWINCH => array('Better_Daemon', 'defaultSigHandler'),
        'SIGPOLL' => array('Better_Daemon', 'defaultSigHandler'),
        SIGIO => array('Better_Daemon', 'defaultSigHandler'),
        'SIGPWR' => array('Better_Daemon', 'defaultSigHandler'),
        'SIGSYS' => array('Better_Daemon', 'defaultSigHandler'),
        SIGBABY => array('Better_Daemon', 'defaultSigHandler'),
        'SIG_BLOCK' => array('Better_Daemon', 'defaultSigHandler'),
        'SIG_UNBLOCK' => array('Better_Daemon', 'defaultSigHandler'),
        'SIG_SETMASK' => array('Better_Daemon', 'defaultSigHandler'),
    );
    ...
    static public function start()
    {
        ...
        if (self::opt('logPhpErrors')) {
            set_error_handler(array('Better_Daemon', 'phpErrors'), E_ALL);
        }
        ...
        if (self::opt('usePEAR')) {
            ...
            if (false === class_exists('Better_Daemon_Exception', true)) {
                // PEAR_Exception is OK. PEAR was found already.
                throw new Better_Exception('Class Better_Daemon_Exception not found');
            }            
        }
        ...
        if (!defined('SIGHUP')) {
            ...
            if (self::opt('usePEAR')) {
                throw new Better_Daemon_Exception($msg);
            } else {
                trigger_error($msg, E_USER_ERROR);
            }
        }        
    ...
    static public function phpErrors($errno, $errstr, $errfile, $errline)
    {
        ...
        // Map PHP error level to Better_Daemon log level
    ...
    public static function emerg()
    {
        ...
        call_user_func_array(array('Better_Daemon', '_ilog'), $arguments);
        return false;
    }
     ...
     *
     * @throws Better_Daemon_Exception  
     * @return boolean
     * @see _logLevels
     * @see logLocation
     */
    ...
    $function = false, $line = false) {
        ...
        if (self::opt('usePEARLogInstance') !== false) {
			Better_Log::getInstance()->logAlert($str, 'daemon');
            return true;
        }
        ...
        $str_date  = '['.Better_Functions::date('M d H:i:s').']';
        ...
        if (!self::opt('logLocation')) {
            throw new Better_Daemon_Exception('Either use PEAR Log or specify '.
                'a logLocation');
        }
    ...
    static protected function _die($restart = false)
    {
        ...
        $pid = file_get_contents(
            Better_Daemon::getOption('appPidLocation')
        );
        @unlink(self::opt('appPidLocation'));
    ...
    static protected function _osObjSetup()
    {
        ...
        if (!self::$_osObj) {
            self::$_osObj = Better_Daemon_OS::factory();
        }
    ...
    static protected function _optionObjSetup()
    {
        // Create Option Object if nescessary
        if (!self::$_optObj) {
            self::$_optObj = new Better_Daemon_Options(self::$_optionDefinitions);
        }
