...
class Better_Daemon_OS
{
    ...
    public function __construct()
    {
        ...
        // Get ancestors
        $ancs = Better_Daemon_OS::_getAncestors($this);
        foreach ($ancs as $i=>$anc) {
            $ancs[$i] = Better_Daemon_OS::_getShortHand($anc);
        }
    ...
    public function &factory($force_os = false, $retried = false)
    {
        $drivers      = array();
        $driversValid = array();
        $class_prefix = "Better_Daemon_OS_";
        ...
        if ($force_os !== false) {
            ...
            $use_name = Better_Daemon_OS::_mostSpecific($driversValid);
        }
        ...
        if (!isset($drivers[$use_name])) {
            ...
            if (!$retried) {
                $obj           = Better_Daemon_OS::factory(false, true);
    ...
    {
        $weights = array_map(
            array("Better_Daemon_OS", "_getAncestorCount"),
            $classes
        );
        arsort($weights);
    ...
    /**
     * Extracts last part of a classname. e.g. Better_Daemon_OS_Ubuntu -> Ubuntu
     *
    ...
    {
        return count(Better_Daemon_OS::_getAncestors($class));
    }
