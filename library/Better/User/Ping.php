 ...
 *
 */
class Better_User_Ping extends Better_User_Base
{
	...
	public function getGlobalSettings()
	{
		if (count($this->settings)==0) {
			...
			if (!$this->settings) {
				$this->settings['game'] = 0;
				$this->settings['direct_message'] = 0;
				$this->settings['request'] = 0;
				$this->settings['friends_shout'] = 0;
				$this->settings['friends_checkin'] = 0;
	...
	public function addQueue(array $params, $type='')
	{
		...
		$this->pingers();		
		$tokens = Better_Phone_Apple::getTokens($this->pingers, $type);
		$pingeds = array();
		...
		if (count($tokens)>0) {
			...
			foreach ($tokens as $row) {
				if (!in_array($row['token'], $pingeds) && $row['uid']) {
					...
					$receiverUserInfo['timezone'] = 8;
					
					$count = (int)$receiver->cache()->get('apns_count');
					$receiver->cache()->set('apns_count', $count+1);
					
					$hour = date('H', time()+(3600*(int)$receiverUserInfo['timezone']));
					
					if ($hour>=$minHour && $hour<=$maxHour) {
						Better_DAO_PingQueue::getInstance()->insert(array(
							'queue_time' => time(),
							'token' => $row['token'],
	...
	public function addQueueForSomebody($receiver, $content, $type='')
	{
		...
		if ($type && $flag) {
			...
			if (isset($pingSettings[$type]) && $pingSettings[$type]) {
				$tokens = Better_Phone_Apple::getTokens((array)$receiver);
				
				if (count($tokens)>0) {
					$count = (int)$receiverUser->cache()->get('apns_count');
					$receiverUser->cache()->set('apns_count', $count+1);
