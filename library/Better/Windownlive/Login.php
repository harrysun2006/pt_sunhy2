...
class Better_Windownlive_Login
{
    ...
    {
        $o = new Better_Windownlive_Login();
        
        $settings = array(
	        'appid' =>'0000000044041708',
	        'secret' => 'lTFfKFZB756bn2ScVuZgzIwF1wn6PoT7',
	        'securityalgorithm' => 'wsignin1.0',
	        'returnurl' => 'http://blog.k.ai/msn/OAuthWrapCallback.php',
	        'policyurl' => 'http://blog.k.ai/live/policy.html'        
        );
    ...
    public function processToken($token, $context=null)
    {
        ...
        try {
            $user = new Better_Windownlive_User(@$parsedToken['ts'], 
                                 @$parsedToken['uid'], 
                                 @$parsedToken['flags'], 
                                 $context, $token);
        } catch (Exception $e) {
    ...
    public function processConsentToken($token, $context=null)
    {
        ...
        try {
            $consenttoken = new Better_Windownlive_Token($this,
                                                @$parsedToken['delt'], 
                                                @$parsedToken['reft'], 
                                                @$parsedToken['skey'], 
                                                @$parsedToken['exp'], 
