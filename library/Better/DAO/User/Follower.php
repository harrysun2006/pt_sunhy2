 ...
 *
 */
class Better_DAO_User_Follower extends Better_DAO_Base
{
    ...
    */
	...
	public function getFollowersDetail($page, $pageSize=BETTER_PAGE_SIZE)
	{
		...
		foreach($servers as $_sid) {
			...
			$select->join(BETTER_DB_TBL_PREFIX.'following AS f', 'f.uid=p.uid AND f.following_uid='.$this->identifier, array('f.dateline'));
