 ...
 *
 */
class Better_DAO_Admin_Denounceuser extends Better_DAO_Admin_Base{
	...
	public function getAllDenounces($params){
		...
		if ($from>0) {
			$select->where('d.dateline>=?', $from);
			$select2->where('d.dateline>=?', $from);
		}
		
		if ($to>0) {
			$select->where('d.dateline<=?', $to);
			$select2->where('d.dateline<=?', $to);
		}
		
		if ($keyword!='') {
			$select->where($rdb->quoteInto('d.denounce_uid=?', $keyword));
			$select2->where($rdb->quoteInto('d.denounce_uid=?', $keyword));
		}
	...
	public function changeStatus($id, $status)
	{
		...
		if($result){
			Better_Admin_Administrators::getInstance(Better_Registry::get('sess')->admin_uid)->addLog('更改举报用户状态为：<br>'.$status, 'change_status');
		}
