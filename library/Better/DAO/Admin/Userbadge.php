<?php
	...
	class Better_DAO_Admin_Userbadge extends Better_DAO_Admin_Base{
		
		public function getAllUserBadges(array $params){
		...
		if (!parent::getDbCacher()->test($cacheKey) || $reload==1) {
			...
			foreach ($serverIds as $sid) {
				...
				$select = $rdb->select();
				$select->from(BETTER_DB_TBL_PREFIX.'user_badges AS ub', '*');
				$select->join(BETTER_DB_TBL_PREFIX.'profile AS p', 'p.uid=ub.uid', array(
				...
				if ($from>0) {
					$select->where('ub.get_time>=?', $from);
				}
				
				if ($to>0) {
					$select->where('ub.get_time<=?', $to);
				}
