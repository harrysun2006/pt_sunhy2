...
class Better_DAO_Poi_Notificationpolo extends Better_DAO_Poi_Base
{
	...
	{
		parent::__construct();
		$this->tbl = BETTER_DB_TBL_PREFIX.'poi_notificationpolo';
		$this->orderKey = 'nid';
		$this->priKey = 'nid';		
	}
	...
	public function &getCoupons(array $params)
	{
		...
		$sql = "SELECT n.nid, n.poi_id, n.title, n.content, n.dateline, n.image_url, n.uid, n.begintm, n.endtm, n.is_top, n.sms_no, n.sms_content, n.url, n.phone
						,p.name
					FROM ".BETTER_DB_TBL_PREFIX."poi_notificationpolo AS n
	...
	public function &search(array $param)
	{
		...
		$select->where('n.nid NOT in(?)', array(1, 2));
		...
		if ($result['total']>0) {
			...
			$select->from($this->tbl.' AS n', array(
				'n.nid', 'n.title', 'n.content', 'n.image_url', 'n.sms_no', 'n.sms_content', 'n.phone', 'n.url', 'n.is_top'
				));
			...
			$select->joinleft(BETTER_DB_TBL_PREFIX.'poi AS p', 'p.poi_id=n.poi_id and n.checked=1', $joinSelected);
	...
	{		
		$sql = "select poi.*,special.*,o.owner from ".BETTER_DB_TBL_PREFIX."poi_notificationpolo AS special left join " .BETTER_DB_TBL_PREFIX."poi as poi on special.poi_id=poi.poi_id left join (select poi_id,group_concat(owner_id) as owner from better_poi_owner group by poi_id) as o  on o.poi_id=special.poi_id where special.nid=".intval($nid)." AND special.begintm<".time()." AND special.endtm>".time();
	...
	{	
		...
		$sql = "select special.* from ".BETTER_DB_TBL_PREFIX."poi_notificationpolo AS special where special.poi_id='".$poi_id."' and checked in(".$checked.")";
		if($checked==1){
