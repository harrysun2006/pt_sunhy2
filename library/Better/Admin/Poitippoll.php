...
class Better_Admin_Poitippoll{
	
	/**
	 * 获得POI点评投票
	 */
	public static function getPoitippolls($params){
		$return = array(
			'count' => 0,
			'rows' => array(),
		);
		...
		$cacheKey = md5($userId.'_'.$blogId.'_'.$poiId.'_'.$from.'_'.$to.'_'.$uid.'_'.'poipoll');
		
		$rows=Better_DAO_Admin_Poitippoll::getInstance()->getAllPOItippolls(array(
			'page' => $page,
			'userId' => $userId,
			'blogId' => $blogId,
			'poiId' => $poiId,
	...
	protected static function parseCheckins($rows){
		$return=array();
		if($rows && count($rows)>0){
			...
			$poi_data=array();
			if(!Better_DAO_Admin_Base::getDbCacher()->test($poi_cacheKey)){
